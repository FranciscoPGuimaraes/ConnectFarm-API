...
from api.models.FinancialModels import FinancialTransaction, FinancialTransactionIn, FinancialTransactionUpdate
from api.services.financial import create_financial, update_financial, read_financial_by_farm_id, delete_financial
...
async def update_financial_endpoint(farm_id: UUID, financial_id: UUID, update_data: FinancialTransactionUpdate):
    try:
        result = await update_financial(farm_id, financial_id, update_data)
        return {"message": f"Financial transaction with id {financial_id} updated successfully!", "result": result}
    except Exception as e:
        raise e

@router.get("/", description="Get all financial transactions for a farm", response_model=List[FinancialTransaction])
...
@router.delete("/{financial_id}", description="Delete a financial transaction from a farm")
async def delete_financial_endpoint(farm_id: UUID, financial_id: UUID):
    try:
        result = await delete_financial(farm_id, financial_id)
        if result:
            return {"message": f"Financial transaction with id {financial_id} deleted successfully!"}
        else:
