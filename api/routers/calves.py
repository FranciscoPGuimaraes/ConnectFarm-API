...
@router.post("/{cattle_number}/calves/create", description="Create one calf in a cattle")
...
async def update_calf_endpoint(farm_id: UUID, calf_number: str, update_data: CalfUpdate):
    try:
...
@router.get("/{cattle_number}/calves/", description="Get all calves from cattle", response_model=List[Calf])
