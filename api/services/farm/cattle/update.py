...
async def update_cattle(farm_id: UUID, matrix_number: int, update_data: CattleUpdate):
    ...
    try:
        ...
        if "weights" in update_fields and update_fields["weights"] is not None:
            ...
            if existing_document and not isinstance(
                existing_document.get("weights", []), list
            ):
                collection.update_one(query, {"$set": {"weights": []}})

            update_operations.setdefault("$push", {}).update(
                {"weights": {"$each": weights}}
            )

        if (
            "reproduction" in update_fields
            and update_fields["reproduction"] is not None
        ):
            ...
            update_operations.setdefault("$push", {}).update(
                {"reproduction": {"$each": reproduction}}
            )

        if (
            "health_history" in update_fields
            and update_fields["health_history"] is not None
        ):
            ...
            update_operations.setdefault("$push", {}).update(
                {"health_history": {"$each": health_history}}
            )
        ...
        if result.matched_count == 0:
            raise HTTPException(
                status_code=404,
                detail=f"Cattle with number {matrix_number} not found in farm {farm_id_str}",
            )

        return {
            "message": f"Cattle with number {matrix_number} in farm {farm_id_str} updated successfully"
        }
