...
async def read_vaccines_cattle(farm_id: UUID, cattle_number: int):
    ...
    try:
        ...
        result = collection.find_one(
            {"farm_id": farm_id, "number": cattle_number}, {"vaccines": 1, "_id": 0}
        )
...
async def read_vaccines_calf(farm_id: UUID, calf_number: str):
    collection, client = connect_mongo("cattles")
    try:
        farm_id = str(farm_id)

        result = collection.find_one(
            {"farm_id": farm_id, "calves.number": calf_number},
            {"calves.$": 1, "_id": 0},
        )
