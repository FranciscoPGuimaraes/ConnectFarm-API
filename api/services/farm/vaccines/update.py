...
async def update_vaccine_cattle(
    farm_id: UUID, cattle_number: int, vaccine_id: str, vaccine_update: VaccineUpdate
):
    collection, client = connect_mongo("cattles")
    try:
        farm_id = str(farm_id)
        update_fields = {
            f"vaccines.$.{key}": value
            for key, value in vaccine_update.dict(exclude_unset=True).items()
        }
...
async def update_vaccine_calf(
    farm_id: UUID, calf_number: str, vaccine_id: str, vaccine_update: VaccineUpdate
):
    collection, client = connect_mongo("cattles")
    try:
        farm_id = str(farm_id)
        update_fields = {
            f"calves.$[calf].vaccines.$[vaccine].{key}": value
            for key, value in vaccine_update.dict(exclude_unset=True).items()
        }
        ...
        if result.modified_count == 0:
            raise HTTPException(
                status_code=404, detail="Cattle, calf, or vaccine not found"
            )
